...
from sec_descriptive_stats import explore_data
...
def run_classification (input_file, output_file, explore_data=True, load_models=False, save_models=False, create_guesser=True):
    ...
    try:
        ...
        if explore_data:
            explore_data(kalpi_data, party_lists, True)
    ...
    if load_models:
        try:
            ...
            print("Decision tree model loaded successfully. Skipping training")
            loaded = True
        except:
    ...
    #if not loaded:
    try:
        print("Optimizing ordinal logistic regression")
    ...
    if load_models:
        try:
            ...
            print("Support vector machine model loaded successfully. Skipping training")
            loaded = True
        except:
    ...
    if load_models:
        try:
            ...
            print("Random forest model loaded successfully. Skipping training")
            loaded = True
        except:
    ...
    if load_models:
        try:
            ...
            print("Neural network model loaded successfully. Skipping training")
            loaded = True
        except:
    ...
    try:
        ...
        scores = numpy.array([dtc_score,omc_score,svc_score,rfc_score,nnc_score])
        #scores = numpy.array([dtc_score,svc_score,rfc_score,nnc_score])
        mean_score = numpy.mean(scores)
        weights = scores/mean_score
        test_weights = numpy.tile(weights, (len(y_test),1))
        test_preds = numpy.transpose(numpy.array([dtc_y_pred,omc_y_pred,svc_y_pred,rfc_y_pred,nnc_y_pred]))
        #test_preds = numpy.transpose(numpy.array([dtc_y_pred,svc_y_pred,rfc_y_pred,nnc_y_pred]))
        ...
        while True:
            ...
            if kalpi_id:
                try:
                    ...
                    print("Decision tree:", numpy.asscalar(dtc_kalpi_pred).astype(int)+1)
                    omc_kalpi_pred = numpy.array([numpy.argmax(x) for x in omc_res_log.model.predict(omc_res_log.params,kalpi_scaled)])
                    print("Ordinal logistic regression:", numpy.asscalar(omc_kalpi_pred).astype(int)+1)
                    svc_kalpi_pred = svc.predict(kalpi_unscaled)
                    print("Support vector machine:", numpy.asscalar(svc_kalpi_pred).astype(int)+1)
                    rfc_kalpi_pred = rfc.predict(kalpi_unscaled)
                    print("Random forest:", numpy.asscalar(rfc_kalpi_pred).astype(int)+1)
                    nnc_kalpi_pred = numpy.array([numpy.argmax(x) for x in nnc.predict(kalpi_scaled, verbose=0)])
                    print("Neural network:", numpy.asscalar(nnc_kalpi_pred).astype(int)+1)
                except:
