...
# Exploring data    
def explore_data_stats(kalpi_data, party_lists, create_graphs=False):
    ...
    jewish_clustered_kalpi_data["bloc_intersection"] = (jewish_clustered_kalpi_data["left_bloc"]-jewish_clustered_kalpi_data["right_bloc"])/(jewish_clustered_kalpi_data["haredi_bloc"]+0.01)

    jewish_means_by_index = jewish_clustered_kalpi_data[["cluster","prop_voters","AMT","B","G","T","KN","L","MHL","MRC","PH","SS","others",\
                                                        "arab_bloc","right_bloc","left_bloc","haredi_bloc","bloc_intersection"]].groupby(["cluster"]).mean().reset_index().sort_values(by=["cluster"])
